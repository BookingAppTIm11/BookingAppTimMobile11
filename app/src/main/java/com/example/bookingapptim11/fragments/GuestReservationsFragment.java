    ...
    private void setupSearchButton(View root) {
        ...
        searchButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                callSearchGuestReservations();
            }
        });
    }

    private void callSearchGuestReservations() {
        ...
        Call<ArrayList<GuestReservation>> call = reservationService.searchGuestReservations(
                checkInSeconds,
                checkOutSeconds,
                accommodationName,
        ...
        call.enqueue(new Callback<ArrayList<GuestReservation>>() {
            ...
            public void onResponse(Call<ArrayList<GuestReservation>> call, Response<ArrayList<GuestReservation>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    reservations.clear();
                    for (GuestReservation reservation : response.body()) {
                        if (spinner.getSelectedItem().equals(reservation.getStatus().toString())) {
                            reservations.add(reservation);
                        } else if(spinner.getSelectedItem().equals("Status (default)")){
                            reservations.add(reservation);
                        }
                    }
                    refreshGuestsReservationsAdapter();
